...
<form action="payment" method="post" class='btn_program'>
    ...
    <? endif?>
    <input type="hidden" name="title_id" value="noprogram">
    <input type="hidden" name="exit_btn">
    <input type="submit" value="Посмотреть другую программу" name="BasketDelete">
</form>

<div class="container main">

    <?if (isset($data['programs'])): ?>
    <h1>Оплата абонемента (30 дней)</h1><br>
    ...
    <div class="products">
        ...
        for($i = 0; $i < count($data['programs']); $i++):
            $sum += $data['programs'][$i]['price'];

            ?>

            <div class="row">
                <img src="/public/img/<?=$data['programs'][$i]['img']?>" alt="<?=$data['programs'][$i]['title']?>">
                <h3>Программа снижения веса "<?=$data['programs'][$i]['title']?>"</h3><br>
                <span>Стоимость абонемента 30 дней <?=$data['programs'][$i]['price']?> BYN</span>
            </div>
        <?php endfor; ?>
        <? endif; ?>
...
<?php else: ?>
    ...
    $fields["wsb_return_url"] = "http://myshop/user/ok";
    $fields["wsb_cancel_return_url"] = "http://myshop/hudeem/error";
    ...
    if(count($data['programs'])){
        ...
        for($i = 0; $i < count($data['programs']); $i++){
            ...
            $fields["wsb_invoice_item_name[$i]"] = $data['programs'][$i]['title'];
            $fields["wsb_invoice_item_quantity[$i]"] = "1";
            $fields["wsb_invoice_item_price[$i]"] = $data['programs'][$i]['price'];
